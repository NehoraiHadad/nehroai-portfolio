...
import { getQuote, listClients } from '@/lib/admin/db/queries';
...
export default async function QuoteDetailPage({ params }: PageProps<'/admin/quotes/[id]'>) {
  ...
  const [dict, initialQuote, savedClients] = await Promise.all([
  ...
  return (
    <>
      ...
      <QuoteBuilder initialQuote={initialQuote} savedClients={savedClients} />
    </>
  );
}
